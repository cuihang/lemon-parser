...
void acttab_action(acttab *p, int lookahead, int action) { // 新建aLookahead 数组。。。
    ...
    if (p->nLookahead == 0) { // 当nLookahead为0.就是此时一个先行符都没有。
        ...
        if (p->mxLookahead < lookahead)
            p->mxLookahead = lookahead; // 如果新进来的先行符在最右边的那个先行符的还要右边,就替换当前最右为新进来的先行符
    ...
    */
    ...
    for (i = 0; i < p->nAction + p->mnLookahead; i++) {
        if (p->aAction[i].lookahead < 0) { // 如果lookahead小于0.一定是-1.。表示这个位置为空
            ...
            if (j < p->nLookahead)
                continue; // 如果运气好,没有阻拦。。那么 这句话就拉不住了。
        ...
        } else if (p->aAction[i].lookahead ==
                   p->mnLookahead) { // 当发现刚送进来的左边界符先行符mnlookahead与历史上的aAction[i].lookahead相同,就看看他们两者的动作是否相同。
            if (p->aAction[i].action != p->mnAction)
                continue; // 动作不相同,不能合二为一。
            ...
            if (j < p->nLookahead)
                continue; // 只有把所有的aLookahead跟aciton进行对比。完全一样才跳过此步。
            n = 0;
...
*/
void myassert(file, line)char *file;
                         int line;
{
    ...
    do { // 上面知道了所有的非终结符可否为空串之后,才能进一步寻找First集合
        ...
        for (rp = lemp->rule; rp; rp = rp->next) {
            ...
            for (i = 0; i < rp->nrhs; i++) { // 进入右边文法符号 进行for循环
                s2 = rp->rhs[i];
                if (s2->type ==
                    TERMINAL) {                                                // 遇到右边的终结符,不用说了,终结符就是s1的First结合中的元素
                    ...
                    if (s1->lambda == B_FALSE)
                        break; // 这时候看左边符号lambda,如果为false,说明它不是一个空串,那么肯定说右边符号的firstset已经有元素了,而且左边等于右边,右边的firstset就是等于左边的first。 不必要再处理了,break跳出
                } else { // 当不是终结符也不是递归是非终结符自身的话没那就要把 右边符号s2的firstset集合中的元素加入到左边符号的firstset集合中去
                    progress += SetUnion(s1->firstset, s2->firstset);
                    if (s2->lambda == B_FALSE)
                        break; // 如果右边的文法符号不可能是空串,说明左边文法符号的firstset已经加好了。 如果右边的该符号可能是空串,那么就需要继续考虑s2后面的各个其他符号的firstset的情况
                }
            }
        }
    } while (progress);
...
void FindStates(
        struct lemon *lemp)                   // 此函数分为三段。第一段是存放项目的项目表x4a的初始化。第二段是找到正确的开始符号。第三段最关键,就是计算得到第一状态state0的基本项目
// struct lemon *lemp;
{
...
does not work properly.",
    ...
    ** left-hand side */
    // 考虑到有些人把开始符号出现在多条产生式的左边
    for (rp = sp->rule; rp; rp = rp->nextlhs) { // nextlhs 存放着 正是左边具有相同非终结符的产生式链条
        struct config *newcfp;
        newcfp = Configlist_addbasis(rp,
                                     0); // Configlist_addbasis是拿来安装基本项目【或者称作核心项目】。。该函数有两个参数,一个是产生式,一个是一个整数。整数表示*符号位于产生式右边的下标值。
...
PRIVATE struct state *getstate(
        struct lemon *lemp)              // 除了第一个状态的基本项目【或者叫做核心项目】,其他状态,包括各个状态中的基本项目和通过闭包运算得到的非基本项目等,都另外由getstate()函数来获取
// struct lemon *lemp;
    ...
    ** by prior calls to "Configlist_addbasis()". */
    ...
    if (stp) { // 该状态已经存在了。
        ...
        ** preexisting state, then return a pointer to the preexisting state */
        ...
        for (x = bp, y = stp->bp;
             x && y; x = x->bp, y = y->bp) {                                  // bp就是同一个状态下的基本项目链表。当x y后面还有节点的的时候
...
// struct state *stp;     /* The state from which successors are computed */
{
    ...
    ** state.  Initially, all configurations are incomplete */
    for (cfp = stp->cfp; cfp; cfp = cfp->next)
        cfp->status = INCOMPLETE; // 全部设置成INCOMPLETE
    ...
    for (cfp = stp->cfp; cfp; cfp = cfp->next) { // stp是状态,cfp是当前状态下面的所有的项目
        ...
        ** construction but with the dot shifted one symbol to the right. */
        for (bcfp = cfp; bcfp; bcfp = bcfp->next) { //
            ...
            if (bsp != sp)
                continue; /* Must be same as for "cfp" */       // 这句话很好理解。sp保留的是某一项目,那么再来一个for循环,这个for循环要干的事就是把sp这个项目 的分隔符往右移动一位。所以必须当前的bsp等于上一个for循环中保留的sp变量。
            bcfp->status = COMPLETE;                            /* Mark this config as used */
            new = Configlist_addbasis(bcfp->rp, bcfp->dot + 1); // 再把该项目加入到 configlist中去
            Plink_add(&new->bplp,
                      bcfp);                        // FIXME 新的项目名字叫做new,它的bplp列表中 应该加入 生成它的这个母体项目  。。这个母体项目就是bcfp、、
        }
        ...
        ** on the symbol "sp" */
        Action_add(&stp->ap, SHIFT, sp,
                   (char *) newstp); // 这四个参数 表示,在原来的stp状态中,由于碰到sp符号,进行移进操作,操作的结果是转移到新的newstp状态。但为了记录这种移进行为,用到了action结构体[就是stp->ap]
    }
}
...
// struct lemon *lemp;
{
    ...
    */
                    ...
                    if (SetFind(cfp->fws, j)) { // cfp 当前处理的那条分隔符在最右边的项目。。。如果返回true,上面在cfp的follow集合中找到了一个终结符
                        ...
                        ** rule "cfp->rp" if the lookahead symbol is "lemp->symbols[j]" */
                        Action_add(&stp->ap, REDUCE, lemp->symbols[j],
                                   (char *) cfp->rp); // FIXME 很重要的原理:只有当产生式上所有符号均已入栈之后,同时,与此项目相关的Follow中有一个终结符,那么就是进行规约的好时机了。
    ...
    if (lemp->start) {
        ...
        if (sp == 0)
            sp = lemp->rule->lhs; // 如果没有%start_symbol的值,那么就取得众多产生式的第一条产生式 的左边符号作为 sp
    } else {
        sp = lemp->rule->lhs;
    }
    ...
    ** start nonterminal.  */
    Action_add(&lemp->sorted[0]->ap, ACCEPT, sp,
               0); // lemp->sorted[0]表示第一状态state0。。这句话就是说为第一个状态state0跟开始符号start_symbol 找到一个accept的状态。。
    ...
    for (i = 0; i < lemp->nstate; i++) {
        ...
        stp->ap = Action_sort(
                stp->ap); // 对动作链 进行排序。。。这里的排序关键看actioncmp函数。规则是:首选,按先行符号的前后进行排序。其次,如果两个后继符号相同,则按此动作类型排序【见结构体e_action】
        for (ap = stp->ap; ap && ap->next; ap = ap->next) { // 紧挨着的循环
            for (nap = ap->next; nap && nap->sp ==
                                        ap->sp; nap = nap->next) { // 对于 nap->sp == ap->sp, 说明 在同一个状态上,对应某一个先行符号,竟然要做不同的动作。 于是,冲突产生了。
    ...
    for (i = 0; i < lemp->nstate; i++) {
        ...
        for (ap = lemp->sorted[i]->ap; ap; ap = ap->next) {
            if (ap->type == REDUCE)
                ap->x.rp->canReduce = B_TRUE; // 计算出所有可以规约的产生式。怎么计算:如果行动上的行为类型是reduce,则伴随着这个动作的产生式肯定可以规约的。
        }
    }
    for (rp = lemp->rule; rp; rp = rp->next) {
        if (rp->canReduce)
...
{ // 看下面的总结信息。送进来函数的apx跟apy两个动作,要么前者移进+后者规约。要么都是规约。
    ...
    if (apx->type == SHIFT && apy->type == REDUCE) {                             // 一、移进规约冲突
        ...
        if (spy == 0 || spx->prec < 0 ||
            spy->prec < 0) { // 情况1、apy的产生式的precsym优先符号不存在 OR apx的先行符号的优先级小于0[没有定义优先级] OR apy的产生式的优先级符号的优先级小于0[没有定义优先级]
        ...
        } else if (spx->prec == spy->prec && spx->assoc ==
                                             RIGHT) { /* Use operator */                             // 后面的都是情况4,就是 前者后者的优先级一样高。那么只能看下它们的 左右结合 性质了。
        ...
        assert(
                ...
                apy->type == CONFLICT); // 下面这段英文仔细看。。不能出现规约移进冲突。【只有可能是移进规约冲突】。。因为移进的排序先于规约的排序
...
struct config *Configlist_add(struct rule *rp,
                              int dot) // 这个函数跟Configlist_addbasis完全一样。但是,它加入的链条仅有current。而Configlist_addbasis加入的链条除了current还有basis
...
struct config *Configlist_addbasis(struct rule *rp,
                                   int dot) // Configtable_insert ==> x4a的结构体,装备config...rp表示那个产生式, dot 就是产生式的"腰斩状态"的两部分的斩点
// struct rule *rp;
// int dot;
{
    struct config *cfp, model;
...
// struct lemon *lemp;
{
    ...
    for (cfp = current; cfp; cfp = cfp->next) { // next字段表示同个状态下不同项目
        ...
        dot = cfp->dot;
        if (dot >= rp->nrhs)
            continue;      // nrhs  产生式右边所有符号的总数,既包括非终止符,也包括终止符... 如果已经到达产生式的最右端, 说明后面没有符号了。闭包也就没有任何项目可加了。
        ...
        if (sp->type == NONTERMINAL) { // 只有非终结符才需要考虑闭包运算。。。。
            ...
            for (newrp = sp->rule; newrp; newrp = newrp->nextlhs) {                                      // nextlhs 存放着 正是左边具有相同非终结符的产生式链条
                newcfp = Configlist_add(newrp,
                                        0); // 由sp作为左边符号的每一个产生式,都可以建立一个分割符在0处的项目。这种项目成为 非基本项目、。。。。至此,我们为闭包增加了一条项目。。此处的0表示分隔符的位置【见example4.out的第3行到5行】
                for (i = dot + 1; i <
                                  rp->nrhs; i++) {                     // TODO 开搞follow集,每个项目,他后面都带了一个follow集合,字段是fws。 // nrhs产生式右边所有符号的总数,既包括非终止符,也包括终止符
                    xsp = rp->rhs[i]; // 取分隔符后面的那个符号。 xsp的index就是 该符号在所有symbol符号数组里面的下标值
                    if (xsp->type == TERMINAL) {
                        SetAdd(newcfp->fws,
                               xsp->index); // 第一种情况,终结符,直接放入follow集合。然后退出for。。 特别强调,char数组,其实就是一个最小整型的数组(一个字节)
                        break;
                    } else {
                        SetUnion(newcfp->fws, xsp->firstset); // 第二种情况,非终结符, 把当前符号xsp的First集合中的所有元素合并到 newcfp的Follow集。
                        if (xsp->lambda == B_FALSE)
                            break; // 假如lambda为false,那么当前符号xsp不可能是空串。它必定有产生式。所以当执行完前面的那个setUnion操作之后,就可跳出循环了,xsp之后的符号不用再考虑了。
                    }
                }
                if (i == rp->nrhs)
                    Plink_add(&cfp->fplp, newcfp); // 如果分割点已经位于 原始分析的产生式的最右边符号了,只需要把新生成的newcfp项目加挂到正在分析的cfp项目的fplp链表中
...
void ErrorMsg(const char *filename, int lineno, const char *format, ...) { // format 是参数格式
    ...
    vsprintf(errmsg, format,
             ap); // int vsprintf(char *string, char *format, va_list param);//将param 按格式format写入字符串string中
    va_end(ap);
    ...
    base = 0;
    while (errmsg[base] != 0) {
        end = restart = findbreak(&errmsg[base], 0,
                                  availablewidth); // findbreak既能让字符串的长度至少有大于min个字符的绝对不多于max个字符,并且让分行得到的字符串的长度尽量达到max
        restart += base;
...
static void handle_D_option(char *z) {
    ...
    azDefine = realloc(azDefine, sizeof(azDefine[0]) *
                                 nDefine); // realloc原型是extern void *realloc(void *mem_address, unsigned int newsize);
...
// char **argv;
{
    ...
    if (rpflag) { // 这个参数 是命令行传进来的,-g, 打印分析结果的统计情况
        ...
        ** links so that the follow-set can be computed later */
        ...
        FindStates(
                &lem);             // TODO 最难最难的代码开始了。、、 寻找第一个状态State0的基本项目;;寻找LR(0)的所有状态;;闭包运算。第七章有详细介绍。。。还好啦,我理解了。。。哈哈。。。
        ...
        if (compress == 0)
            CompressTables(&lem); // 9.4节压缩动作链表

        /* Generate a report of the parser generated.  (the "y.output" file) */
        if (!quiet)
            ReportOutput(&lem); // 9.5节报告动作链表
        ...
        FILE *err;
{
    ...
    spcnt = 0;
    if (argv[0])
        fprintf(err, "%s", argv[0]); // 打印第0个参数,就是程序的全名
    spcnt = strlen(argv[0]) + 1;
    for (i = 1; i < n && argv[i]; i++) {
        fprintf(err, " %s", argv[i]);
        spcnt +=
                strlen(argv[i]) + 1; // 原来的代码是spcnt += strlen(argv[i]+1); 这明显是一个bug. 在3.2.2 中已经得到修复了。本代码是3.1.2,所以还是一个bug
    }
    spcnt += k;
    for (; argv[i]; i++)
        fprintf(err, " %s", argv[i]);
    ...
    if (argv != 0 && *argv != 0) {
        for (i = 1; argv[i]; i++) { // argv[0] 是程序的全名,要避开
            if (dashdash || !ISOPT(argv[i])) {
                if (n == 0)
                    return i; // 当n=0,表示要取得语法.y文件的全路径名称
                n--;
            }
            if (strcmp(argv[i], "--") == 0)
                dashdash = 1; // 这是废话、。不可能执行到这里
        }
    }
    return -1;
}
        ...
        FILE *err;
{
    ...
    for (j = 0; op[j].label; j++) {
        if (strncmp(&argv[i][1], op[j].label, strlen(op[j].label)) == 0)
            break; // strncmp(const char * str1, const char * str2, size_t n)函数,若str1与str2的前n个字符相同，则返回0；若s1大于s2，则返回大于0的值；若s1 若小于s2，则返回小于0的值。
    }
    v = argv[i][0] == '-' ? 1 : 0;
    if (op[j].label == 0) { // label 是第1个属性[下标0开始],那些b c g m
        ...
        *((int *) op[j].arg) = v; // arg是s_options结构体的第2个属性[下标0开始],代表值
        ...
        (*(void (*)()) (op[j].arg))(v);
               ...
               OPT_FSTR) {                                            // TODO 主要是D参数。。。后面再讨论.OPT_FSTR类型的s_options结构体的arg是一个函数指针
        (*(void (*)()) (op[j].arg))(
                &argv[i][2]); // -D123,因为前面一定是"-D",所以用argv[i][0]是'-',argv[i][1]是'D',那么&argv[i][2]就是123的开始位置了
    ...
    *cp = '='; // 比较完了之后,回复老样子
    if (op[j].label == 0) {
        if (err) { // 逻辑同handleflags,如果一直找不到对应的属性[b c g m q s x], 那么最后的op[j]就是 {OPT_FLAG,0,0,0}, 那么就报错了
            fprintf(err, "%sundefined option.\n", emsg);
            errline(i, 0,
                    err); // 为什么参数是0,而handleflags函数的errline参数是1. 因为这里的D参数前面没有中划线。而handleflags函数的-b -m -g 参数,前面有个中划线。所以报错指明的"^-- here"标志的时候有所不同
        }
        errcnt++;
    } else {
        cp++;
...
int OptNArgs() {
    ...
    if (argv != 0 && argv[0] != 0) {
        for (i = 1; argv[i]; i++) { // gcc中,main函数进来的argv参数组,能保证最后一个NULL的地址一定是0x00
            if (dashdash || !ISOPT(argv[i]))
                cnt++; // 不是opt的话,就当成是传进来的file路径
            if (strcmp(argv[i], "--") == 0)
                dashdash = 1; // strcmp 比较两个字符串..这里感觉没有毛用,因为'--'一定死在前面的OptInit,走不到这一步
        }
    }
...
static void parseonetoken(psp)struct pstate *psp;
{
    char *x;
...
to follow the previous rule.");
        ...
        case WAITING_FOR_PRECEDENCE_SYMBOL:
            if (x[0] == '.') {
                ...
                struct symbol *sp;
                sp = Symbol_new(
                        x); // 大写的话就是终结符,开始写入 Symbol结构体,这个结构体的 全局变量就是x2a.. x2a 可以存放终结符或者非终结符。。此处没有出现小黑点,所以下个状态还是寻找优先符
                if (sp->prec >= 0) {
                    ErrorMsg(psp->filename, psp->tokenlineno,
...
static void preprocess_input(char *z) { // 处理预编译 代码%ifdef-%endif
    ...
    for (i = 0; z[i]; i++) {
        ...
        if (strncmp(&z[i], "%endif", 6) == 0 && isspace(z[i +
                                                          6])) { // int strncmp ( const char * str1, const char * str2, size_t n );参数】str1, str2 为需要比较的两个字符串，n为要比较的字符的数目。
            ...
            if (exclude) {
                ...
                } // 跨越所有的空格 比如%ifdef      first这样,忽略中间所有的空格
                for (n = 0; z[j + n] && !isspace(z[j + n]); n++) {
                } // 跨越空格 之后的那个值的所有非空格 比如%ifdef      first这样, 得到first整个单词的长度值n
                exclude = 1;
                for (k = 0; k < nDefine; k++) { // nDefine就是那个 从命令行参数 传进来的那些D参数的 总个数
                    if (strncmp(azDefine[k], &z[j], n) == 0 &&
                        strlen(azDefine[k]) == n) { // 如果这货 已经存在 D参数列表里面了的话,就跳出循环。 那么排除个数exclude就置为0
                        exclude = 0;
                        break;
                    }
                }
                if (z[i + 3] == 'n')
                    exclude = !exclude; // 1.1 属于%ifndef的话, 并且D参数没有这货,那么 exclude 此时的值是0,表示不用排除这代码. 1.2 属于%ifndef的话, 并且D参数有这货,那么 exclude 此时的值是1,那得排除这段代码
            ...
            for (j = i; z[j] && z[j] != '\n'; j++)
                z[j] = ' '; // %ifndef 或者 %ifdef 虽然后面是空格,但是在同一行中还有其他东西的东西,比如"%ifdef 123 234"之类的,把后面的"123 234"删除
        }
    }
...
void Parse(struct lemon *gp) // 理解成scanner 或者 lexer 。。。其实准确说,他不是一个Parser,这点要注意
{
    ...
    fseek(fp, 0,
          2);                        //指向文件尾巴。 int fseek(FILE *stream, long offset, int fromwhere); 如果执行成功，指针将指向以fromwhere【偏移起始位置：文件头0(SEEK_SET)，当前位置1(SEEK_CUR)，文件尾2(SEEK_END)】为基准，偏移offset（指针偏移量）个字节的位置。
...
{ // 这个函数一句话概括:只要没有碰到%%,就从"in源" 一行一行 全部copy再粘贴到 "out去处"。
    ...
    while (fgets(line, LINESIZE, in) &&
           (line[0] != '%' || line[1] != '%')) { // 这句while一定先执行fgets读取1000个字符到line,再判断line[0] 跟 line[1]
        (*lineno)++;
        iStart = 0;
        if (name) {
            for (i = 0; line[i]; i++) {
...
{                                                       // 解决的核心思想,就是用 C语言的联合类型。
    ...
    for (i = 0; i < lemp->nsymbol; i++) {
        ...
        if (sp->datatype == 0)
            continue; // datatype是一个字符串,安放着这个符号的数据类型信息。
        len = strlen(sp->datatype);
        if (len > maxdtlength)
            maxdtlength = len; // 继续计算maxdtlength值
    }
    ...
    */
    for (i = 0; i < lemp->nsymbol; i++) {
        ...
        if (sp->type != NONTERMINAL || (sp->datatype == 0 && lemp->vartype ==
                                                             0)) {                  // 终结符,或者是【非终结符,但是此非终结符的datatype是null,并且y文件的默认datatype也是null】
            sp->dtnum = 0; // dtnum就是data type num的意思。。
            continue;
        }
        cp = sp->datatype; // 有自己的数据类型的非终结符。。。
        if (cp == 0)
            cp = lemp->vartype; // 该终结符本身没有数据类型,但是 y文件有定义一个 默认的全局数据类型vartype
        j = 0;                  // 经过以上几步的处理。所有符号【除了特殊的error符号】都应该有自己的数据类型了。
        while (isspace(*cp))
            cp++; // 忽略前面的空格符号
        while (*cp)
            stddt[j++] = *cp++; // 从非空格符号开始,全部拷贝到stddt数组里面。
...
// int mhflag;     /* Output in makeheaders format if true */
{
    ...
    if (lemp->arg &&
        lemp->arg[0]) { // lemp->arg 是 %extra_argument申明符 带进来的。。。sqlite3的y文件就有这货。%extra_argument {Parse *pParse}
        int i;
        i = strlen(lemp->arg);
    ...
    */
    ...
    for (i = 0; i < lemp->nstate; i++) {
        ...
        for (ap = stp->ap; ap; ap = ap->next) { // ap就是此状态下的动作链表,就是所谓的分析链表
            if (compute_action(lemp, ap) >=
                0) { // 移进 规约 错误 接受 就会返回一个值,该值不小于0。。TODO compute_action很简单,就是计算下当前action先行符的index
    ...
    */
    ...
    for (i = 0;
         i < lemp->nstate * 2 && ax[i].nAction > 0; i++) { // 由于ax是排过序的,所以 当遇到nAction的值是0的时候,后面的那些ax元素的nAction值肯定都是0
        stp = ax[i].stp;
        if (ax[i].isTkn) { // isTkn表示这个ax是 跟终结符相关的 结构体。
            for (ap = stp->ap; ap; ap = ap->next) {
                int action;
                if (ap->sp->index >= lemp->nterminal)
                    continue; // 先行符不是终结符就忽略
                action = compute_action(lemp, ap);
                if (action < 0)
                    continue;                                  // 当action值小于0,说明不是 移进 规约 出错 接受中的任何一种。conflict sh_resolved rd_resolved not_used 都被过滤了
    ...
    for (i = j = 0; i < n; i++) {
        ...
        if (action < 0)
            action = lemp->nsymbol + lemp->nrule + 2; // 不存在,就使用nsymbol_nrule+2这个大值 来赋值。。
        if (j == 0)
            fprintf(out, " /* %5d */ ", i);
    ...
    */
    ...
    tplt_xfer(lemp->name, in, out,
              &lineno);                          // 10、继续去lempar.c文件里面读数据 拷贝过来 输出到example.cpp.因为 第10个双百分号的出现而stop
    ...
    */
    ...
    tplt_xfer(lemp->name, in, out,
              &lineno);                    // 14、继续去lempar.c文件里面读数据 拷贝过来 输出到example.cpp.因为 第14个双百分号的出现

    /* Generate code which executes when the parser accepts its input */
    tplt_print(out, lemp, lemp->accept, lemp->acceptln, &lineno); // //第14个双百分号:输出accept code
    tplt_xfer(lemp->name, in, out,
              &lineno);                      // 没有第十五个双百分号了。15、继续去lempar.c文件里面读数据 拷贝过来 输出到example.cpp. 最后就是lempar.c文件的结尾了。
...
// struct lemon *lemp;
{
    ...
    for (i = 0; i < lemp->nstate; i++) {
        ...
        for (ap = stp->ap; ap; ap = ap->next) {
            ...
            for (ap2 = ap->next; ap2; ap2 = ap2->next) {
                ...
                if (rp2 == rbest)
                    continue;
                if (rp2 == rp)
                    n++; // 当发现两条产生式是一样的时候,。。就是说,我们在同一状态中,发现了具有同一产生式的两个规约动作
            }
            if (n > nbest) {
                nbest = n;
                rbest = rp;
        ...
        ** is not at least 2 */
        if (nbest < 2)
            continue; // 如果最佳产生式的数量少于2,即是说所有的规约动作分别 只有一条产生式的时候,就 无所谓默认的产生式了。
        ...
        for (ap = ap->next; ap; ap = ap->next) {
            if (ap->type == REDUCE && ap->x.rp == rbest)
                ap->type = NOT_USED; // 把reduce到最佳产生式的那些动作 的type都改成NOT_USED
        }
...
// char *s2;
{
    ...
    for (i = 0; i < size; i++) {
        if (s2[i] == 0)
            continue; // 先判断第i个终结符是否已在s2的firstset集合中。如果s2[i]为0,表示这个终结符不在集合中,不用考虑添加。
...
void Strsafe_init() {
    ...
    if (x1a) {
        ...
        x1a->tbl = (x1node *) malloc(                     // sizeof(x1node)=24个字节 sizeof(x1node*)=8个字节。一般C语言一个指针就是4个字节=32bits,但是 在MAC64位机器上就是8个字节=64bits。 也就是8个char,在c语言中,一个char就是一个byte
                ...
                1024); // √TODO 为什么是(sizeof(x1node) + sizeof(x1node*))*1024 而不是 (sizeof(x1node))*1024? 要结合下面的代码来理解x1a->ht
        if (x1a->tbl == 0) {
            free(x1a);
            x1a = 0;
        } else {
...
int Symbolcmpp(struct symbol **a, struct symbol **b) { // 所以通过这个函数实现。
    ...
    return i1 -
           i2; // 简单说下该算法,让每一个符号的第一个字母跟大写的"Z"进行比较,如果ASCII值比大写"Z"还大,说明是小写字母,否则是大写字母。当是小写字母的时候,得到"真"值,把值放大一千倍倍,所以小写字母的数值都很大了。
...
// struct config *key;
{
    ...
    while (np) {
        if (Configcmp(np->data, key) == 0)
            break; // Configcmp函数,就先比较产生式的index,再比较dot小黑点的位置
        np = np->next;
    }
    return np ? np->data : 0;
}
